...
def balanced_accuracy(predictions, output_data):
    TP = 0
    FP = 0
    TN = 0
    FN = 0
    for i in range(len(predictions)):
        if predictions[i] == output_data[i] == 1:
            TP += 1
        elif predictions[i] == output_data[i] == 0:
            TN += 1
        elif predictions[i] == 1:
            FP += 1
        elif predictions[i] == 0:
            FN += 1
        elif output_data[i] == 0:  # when prediction is -1
            FP += 1
        elif output_data[i] == 1:  # when prediction is -1
            FN += 1
...
def evaluate(param, instructions, input_data, output_data):
    ...
    # if the program has no effective instructions then it gets all samples incorrect
        ...
        predictions = [-1] * len(output_data)

    error_vect = [1 if predictions[i] == output_data[i] else 0 for i in range(len(predictions))]
    return [balanced_accuracy(predictions, output_data), repr(error_vect), predictions]
